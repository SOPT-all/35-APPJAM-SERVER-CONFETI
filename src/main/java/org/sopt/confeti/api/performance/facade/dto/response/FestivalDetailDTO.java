...
public record FestivalDetailDTO(
    ...
    public static FestivalDetailDTO of(final Festival festival, boolean isFavorite, final S3FileHandler s3FileHandler) {
        return new FestivalDetailDTO(
                ...
                s3FileHandler.getFileUrl(FolderPath.combine(FolderPath.FESTIVAL, FolderPath.POSTER), festival.getFestivalPosterPath()).toString(),
                s3FileHandler.getFileUrl(FolderPath.combine(FolderPath.FESTIVAL, FolderPath.POSTER_BG), festival.getFestivalPosterBgPath()).toString(),
                ...
                s3FileHandler.getFileUrl(FolderPath.combine(FolderPath.FESTIVAL, FolderPath.MAIN_BANNER), festival.getFestivalReservationBgPath()).toString(),
                s3FileHandler.getFileUrl(FolderPath.combine(FolderPath.FESTIVAL, FolderPath.LOGO), festival.getFestivalLogoPath()).toString(),
